<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
</head>
<body>
    <nav class="nav_bar">
        <img src="../images/deakens_logo.png" alt="" width=60 heigth=60>
        <ul>
            <li><a href="../Dossiers_projet/accueil.php"><i class="fas fa-tachometer-alt"></i>Tableau de bord</a></li>
            <li><a href="../Dossiers_projet/dossier.php"><i class="far fa-edit"></i>Enregistrer un dossier</a></li>
            <li><a href="../Dossiers_projet/rapport.php"><i class="fas fa-tasks"></i>rapports</a></li>
            <li><a href="../Dossiers_projet/clientsTraites.php"><i class="fas fa-clipboard-check"></i>Clients traités</a></li>
            <li><a href="../Dossiers_projet/agence.php"><i class="fas fa-user-edit"></i>Créer Agence</a></li>
            <li><a href="../Dossiers_projet/projet.php"><i class="fas fa-project-diagram"></i>Créer un projet</a></li>
            <li><a href="../Utilisateurs/enregistrerUtilisateur.php"><i class="fas fa-user-tag"></i>utilisateurs</a></li>
            <li><a href="../Utilisateurs/deconnexion.php"><i class="fas fa-sign-out-alt"></i>logout</a></li>
        </ul>
    </nav>
</body>
</html>